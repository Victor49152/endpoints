...
def setup_benchmark(config: BenchmarkConfig, test_mode: TestMode) -> BenchmarkContext:
    ...
    # Tokenizer: use explicit tokenizer_name if set, otherwise fall back to model name.
    # Set tokenizer_name in model_params when the serving model path/name differs
    # from the tokenizer (e.g. quantized checkpoints, container-local paths).
    tokenizer_source = config.model_params.tokenizer_name or config.model_params.name
